...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	{
		this->_hitPoints = 0;
		std::cout << GREEN << "-ClapTrap- " << this->_name << " took " << amount << " points of damage and died!" << RESET << std::endl;
	}
	else
	{
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (this->_hitPoints < 1)
	{
		std::cout << GREEN << "-ClapTrap- " << this->_name << " is dead and can't be repaired!" << RESET << std::endl;
		return;
	}
	if (this->_hitPoints >= 10)
	{
		std::cout << GREEN << "-ClapTrap- " << this->_name << " is already at full health!" << RESET << std::endl;
		return;
	}
	if (this->_energyPoints < 1) 
	{
